...
def _normalize_column_name(col: str) -> str:
    ...
    aliases = {
        ...
        "network_carrier": "carrier_name",
        "mobile_carrier": "carrier_name",
        "sim_carrier": "carrier_name",
        "sim_carrier_name": "carrier_name",
        "siminfo": "sim_info",
        "sim": "sim_info",
        "sim_data": "sim_info",
        ...
        "activate_date": "activation_date",
        "date_activated": "activation_date",
        "office_date": "activation_date",
        ...
        "defect_damage": "defect_damage_type",
        "responsible": "responsible_party",
        "party_responsible": "responsible_party",
        "agegroup": "age_range",
        "age_group": "age_range",
        "age_bucket": "age_range",
        "agerange": "age_range",
        "app_uage": "app_usage",
        "app_uage_s": "app_usage",
        "app_usage_s": "app_usage",
        "app_usage_seconds": "app_usage",
        "application_usage": "app_usage",
        "apps_usage": "app_usage",
    }
...
def _normalize_dataframe_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if df.columns.duplicated().any():
        ...
        logger.warning(f"⚠️ Duplicate columns after normalization collapsed: {duplicate_cols}")
...
def insert_dashboard_data(df, upload_file):
    ...
    try:
        ...
        if df.empty:
            raise ValueError("No valid columns found in uploaded dashboard file after normalization.")
        ...
        # Attach metadata
        df.loc[:, 'upload_file'] = upload_file
        df.loc[:, 'upload_timestamp'] = datetime.utcnow()

        df = df.astype(object).where(pd.notnull(df), None)
        records = [
            [_clean_db_value(value) for value in row]
        ...
        update_clause = ', '.join([
            f'"{col}" = EXCLUDED."{col}"' for col in insert_cols if col != 'device_number'
        ])
        ...
        execute_batch(cursor, insert_stmt, records)
        conn.commit()
    except Exception:
        conn.rollback()
        raise
...
def upload_dashboard_data():
    ...
    try:
        ...
        logger.exception(f"❌ Error uploading dashboard data: {e}")
        return jsonify({"error": str(e)}), 500
    finally:
...
def calculate_app_usage_percentages(df: pd.DataFrame, column_name: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for entry in df[column_name]:
            ...
            try:
                # JSON formats, e.g. {"Facebook": 120, "Instagram": 90}
                # or [{"app":"Facebook","time":120}, ...]
                json_candidate = entry.strip()
                if json_candidate.startswith("{") or json_candidate.startswith("["):
                    try:
                        parsed = json.loads(json_candidate)
                        if isinstance(parsed, dict):
                            for k, v in parsed.items():
                                app_name = str(k).strip()
                                value_match = re.search(r"\d+", str(v))
                                if app_name and value_match:
                                    app_usage[app_name] = app_usage.get(app_name, 0) + int(value_match.group(0))
                            continue
                        if isinstance(parsed, list):
                            ...
                            for item in parsed:
                                if not isinstance(item, dict):
                                    continue
                                app_name = str(item.get("app") or item.get("name") or item.get("application") or "").strip()
                                raw_time = item.get("time") or item.get("seconds") or item.get("usage") or item.get("duration")
                                value_match = re.search(r"\d+", str(raw_time)) if raw_time is not None else None
                                if app_name and value_match:
                                    app_usage[app_name] = app_usage.get(app_name, 0) + int(value_match.group(0))
                                    consumed = True
                            if consumed:
                ...
                for app in apps:
                    ...
                    # Format 2: "Facebook:120" / "Facebook=120"
                    kv_match = re.match(r"^(.+?)\s*[:=]\s*(\d+)", app)
                    if kv_match:
                        app_name = kv_match.group(1).strip()
                        time = int(kv_match.group(2))
                        if app_name:
                            app_usage[app_name] = app_usage.get(app_name, 0) + time
                        continue

                    # Format 3 fallback: app list without durations -> count as 1 occurrence
                    app_name = app.strip()
                    if app_name:
...
def get_dashboard_data():
    ...
    try:
        ...
        ## ======== CHURN ANALYSIS ========
        churn_date_col = _first_existing_column(df, ["activation_date", "office_date", "created_date", "created_at", "month"])
        if churn_date_col and "churn" in df.columns:
        ...
        age_col = _first_existing_column(df, ["age_range", "age_group", "age_bucket", "agerange", "age"])
        if age_col:
            if age_col == "age":
            ...
            if series.dropna().empty or (series.astype(str).str.strip() == "").all():
                birth_col = _first_existing_column(df, ["dob", "date_of_birth", "birth_date", "birthday"])
                if birth_col:
                    birth_dt = pd.to_datetime(df[birth_col], errors='coerce')
                    age_years = (pd.Timestamp.now() - birth_dt).dt.days / 365.25
                    bins = [0, 18, 25, 35, 45, 55, 65, 200]
                    labels = ["0-17", "18-24", "25-34", "35-44", "45-54", "55-64", "65+"]
                    series = pd.cut(age_years, bins=bins, labels=labels, right=False)
        else:
            birth_col = _first_existing_column(df, ["dob", "date_of_birth", "birth_date", "birthday"])
            if birth_col:
                birth_dt = pd.to_datetime(df[birth_col], errors='coerce')
        ...
        ## ======== ACTIVATION COUNTS ========
        activation_col = _first_existing_column(df, ["activation_date", "activated_date", "activation_month", "month", "office_date"])
        if activation_col:
            ...
            for month, count in activation_counts_raw.items():
                ...
                activation_counts.append({
                    "month": _format_month_label(month),
                    "count": int(count),
        ...
        ## ======== APP USAGE ANALYSIS ========
        app_usage_col = _first_existing_column(df, [
            "app_usage",
            "app_usage_time",
        ])
        if app_usage_col:
            dashboard_metrics["app_usage_percentages"] = calculate_app_usage_percentages(df, app_usage_col)
...
@app.route('/dashboard_columns', methods=['GET'])
def dashboard_columns():
    """Returns dashboard_devices column names to help frontend/backend schema alignment."""
    try:
        conn = psycopg2.connect(**DB_CONFIG)
        cursor = conn.cursor()
        cursor.execute("""
        ...
        logger.warning(f"⚠️ DB unavailable for /dashboard_columns: {e}")
        return jsonify({"columns": []})
    except Exception as e:
        logger.error(f"❌ Error extracting dashboard columns: {e}")
        return jsonify({"error": str(e)}), 500
...
def carrier_distribution():
    ...
    try:
        ...
        carrier_col = _first_existing_column(
            pd.DataFrame(columns=list(existing_cols)),
            ["sim_info", "carrier_name"]
        )
        if not carrier_col:
            ...
            for sim_info in df['sim_info'].dropna():
                try:
                    ...
                    for sim in sim_data:
                        if not sim:
                            continue
                        raw_name = sim.get('carrier_name') or sim.get('carrier') or sim.get('network')
                        carrier_name = normalize_carrier_name(raw_name)
                        if carrier_name:
                            carrier_counts[carrier_name] = carrier_counts.get(carrier_name, 0) + 1
                except (TypeError, json.JSONDecodeError):
                    carrier_name = normalize_carrier_name(sim_info)
                    if carrier_name:
                        carrier_counts[carrier_name] = carrier_counts.get(carrier_name, 0) + 1

        if not carrier_counts and "carrier_name" in df.columns:
            for raw_name in df['carrier_name'].dropna():
                carrier_name = normalize_carrier_name(raw_name)
                if carrier_name:
                    carrier_counts[carrier_name] = carrier_counts.get(carrier_name, 0) + 1
...
def time_analysis():
    try:
        ...
        interval_col = _first_existing_column(pd.DataFrame(columns=list(existing_cols)), ["interval_date"])
        last_boot_col = _first_existing_column(pd.DataFrame(columns=list(existing_cols)), ["last_boot_date"])
        ...
        date_columns = [interval_col, last_boot_col, active_col]
        for col in date_columns:
            df[col] = pd.to_datetime(df[col], errors='coerce')
        
        # Calculate data for the graph.
        df['usage_duration'] = (df[last_boot_col] - df[active_col]).dt.days
